...
<head>
    ...
    <style>x`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        th,td{padding:11px 12px;font-size:13px;text-align:left;border-bottom:1px solid #f1f5f9;vertical-align:middle;}
        th{font-weight:700;background:#f9fafb;}
        img.thumb{width:44px;height:44px;object-fit:cover;border:1px solid #e5e7eb;}
...
<body>
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('admin.products.index') }}" class="active">📦 Semua Produk</a>
    ...
    <main class="main">
        <div class="head"><h1>Semua Produk ({{ $products->total() }})</h1></div>
        ...
        <div class="card">
            <table>
                <thead><tr><th>Foto</th><th>Nama</th><th>Toko Mitra</th><th>Kategori</th><th>Harga</th><th>Status</th><th>Aksi</th></tr></thead>
                <tbody>
                    @foreach($products as $p)
                    <tr>
                        <td><img class="thumb" src="{{ $p->image_url }}" alt="{{ $p->name }}"></td>
                        <td><strong>{{ $p->name }}</strong><br><small style="color:#6b7280;">{{ $p->brand }}</small></td>
                        <td>{{ $p->partner?->store_name ?? '—' }}</td>
                        ...
                        <td style="display:flex;gap:4px;">
                            <form method="POST" action="{{ route('admin.products.suspend', $p) }}">
                                @csrf @method('PUT')
                                <button type="submit" class="btn btn-suspend">{{ $p->is_active ? 'Nonaktifkan' : 'Aktifkan' }}</button>
                            </form>
