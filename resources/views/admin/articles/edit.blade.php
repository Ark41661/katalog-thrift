            ...
            <form method="POST" action="{{ route('admin.articles.update', $article) }}" enctype="multipart/form-data">
                ...
                <div class="section">
                    ...
                    @if($article->cover_image_url)
                        <img src="{{ $article->cover_image_url }}" alt="Cover saat ini" style="max-width:200px;max-height:120px;object-fit:cover;border:1px solid #e5e7eb;display:block;margin-bottom:8px;">
                    @elseif($article->cover_image)
                        <div style="background:#fef3c7;color:#92400e;padding:10px 12px;font-size:12px;margin-bottom:10px;border:1px solid #fde68a;">⚠️ Cover saat ini tidak valid (bukan file gambar). Upload file baru di bawah.</div>
                    @endif
                    <div style="display:flex;gap:0;margin-bottom:10px;border:1px solid #e5e7eb;">
                        <button type="button" id="tab-file-btn" onclick="switchCoverTab('file')" style="flex:1;padding:8px;font-size:12px;font-weight:700;border:0;background:#111827;color:#fff;cursor:pointer;">Upload File Baru</button>
                    ...
                    <div id="cover-url-wrap" style="display:none;">
                        <input type="url" name="cover_image" value="{{ old('cover_image', str_starts_with($article->cover_image ?? '', 'http') ? $article->cover_image : '') }}" placeholder="https://..." oninput="previewCoverUrl(this.value)">
                        <img id="cover-url-preview" src="{{ $article->cover_image_url }}" alt="Preview" style="margin-top:8px;max-width:200px;max-height:120px;object-fit:cover;border:1px solid #e5e7eb;{{ $article->cover_image_url ? '' : 'display:none;' }}">
                    </div>
                    <label>Excerpt</label>
                    <textarea name="excerpt" style="min-height:60px;" maxlength="300">{{ old('excerpt', $article->excerpt) }}</textarea>
                </div>
                <div class="section">
                    <p class="section-title">Isi Artikel</p>
                    <p class="hint" style="margin-bottom:10px;">Tulis dengan teks biasa. Enter = baris baru.</p>
                    <textarea name="content" style="min-height:400px;" required oninput="updatePreview(this.value)">{{ old('content', $article->content) }}</textarea>
                    <p class="hint">Preview:</p>
                    <div class="preview-box" id="preview">{{ $article->content }}</div>
                </div>
                <div class="section">
                    ...
                    <div class="checkbox-row">
                        ...
                        <label for="is_published">Terbitkan (tampil ke publik)</label>
                    </div>
                </div>
