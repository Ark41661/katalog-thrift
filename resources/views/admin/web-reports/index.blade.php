...
<head>
    ...
    <style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .head .tabs{display:flex;gap:8px;margin-top:12px;}
        .head .tabs a{padding:6px 14px;font-size:12px;font-weight:700;background:#fff;border:1px solid #e5e7eb;text-decoration:none;color:var(--muted, #6b7280);}
        .head .tabs a.active{background:#111827;color:#fff;border-color:#111827;}
        ...
        .cat-label{font-size:11px;color:#6b7280;}
    </style>
</head>
<body>
    <aside class="sidebar">
    ...
    <main class="main">
        <div class="head">
            <h1>Laporan Web ({{ $reports->total() }})</h1>
            <div class="tabs">
                <a href="{{ route('admin.web-reports.index', ['status' => 'pending']) }}" class="{{ $activeStatus === 'pending' ? 'active' : '' }}">Pending</a>
                <a href="{{ route('admin.web-reports.index', ['status' => 'resolved']) }}" class="{{ $activeStatus === 'resolved' ? 'active' : '' }}">Selesai</a>
                <a href="{{ route('admin.web-reports.index', ['status' => 'ignored']) }}" class="{{ $activeStatus === 'ignored' ? 'active' : '' }}">Diabaikan</a>
                <a href="{{ route('admin.web-reports.index', ['status' => 'all']) }}" class="{{ $activeStatus === 'all' ? 'active' : '' }}">Semua</a>
            </div>
        </div>
        ...
        <div class="card">
            <table>
                <thead><tr><th>Pelapor</th><th>Kategori</th><th>Pesan</th><th>Status</th><th>Tanggal</th><th>Aksi</th></tr></thead>
                <tbody>
                    @forelse($reports as $r)
                    <tr>
                        <td>{{ $r->name }}<br><small style="color:#6b7280;">{{ $r->email }}</small></td>
                        <td><span class="cat-label">{{ str_replace('_', ' ', ucfirst($r->category)) }}</span></td>
                        <td style="max-width:200px;font-size:12px;color:#374151;">{{ Str::limit($r->message, 80) }}</td>
                        <td><span class="badge badge-{{ $r->status }}">{{ ucfirst($r->status) }}</span></td>
                        <td>{{ $r->created_at->format('d M Y') }}</td>
                        <td>
                            <a href="{{ route('admin.web-reports.show', $r) }}" class="btn" style="background:#111827;color:#fff;margin-bottom:4px;">Detail</a>
                            @if($r->status === 'pending')
                            <div style="display:flex;gap:4px;margin-top:4px;">
                                <form method="POST" action="{{ route('admin.web-reports.resolve', $r) }}">
                                    @csrf @method('PUT')
                                    <button type="submit" class="btn btn-resolve">✓ Selesai</button>
                                </form>
                                <form method="POST" action="{{ route('admin.web-reports.ignore', $r) }}">
                                    @csrf @method('PUT')
                    ...
                    @empty
                    <tr><td colspan="6" style="text-align:center;padding:20px;color:#6b7280;">Tidak ada laporan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
