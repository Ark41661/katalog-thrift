...
<head>
    ...
    <title>Notifikasi - {{ $partner->store_name }}</title>
    <style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .btn-small{padding:8px 14px;font-size:12px;font-weight:600;background:#fff;border:1px solid #e5e7eb;cursor:pointer;}
...
<body>
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('partner.notifications') }}" class="active">🔔 Notifikasi</a>
    ...
    <main class="main">
        <div class="head">
            ...
            @if($notifications->where('read_at', null)->count() > 0)
                <form method="POST" action="{{ route('partner.notifications.read-all') }}">
                    @csrf
        ...
        @else
            <div class="list">
                ...
                <div class="item {{ $n->read_at ? '' : 'unread' }}">
                    ...
                    @unless($n->read_at)
                        <form method="POST" action="{{ route('partner.notification.read', $n->id) }}">
                            @csrf
