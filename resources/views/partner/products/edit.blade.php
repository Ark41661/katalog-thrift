...
<head>
    ...
    <title>Edit Produk - {{ $partner->store_name }}</title>
    <style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .preview-img{margin-top:8px;max-width:180px;max-height:140px;object-fit:cover;border:1px solid #e5e7eb;}
...
<body>
    ...
    <main class="main">
        <div class="form-wrap">
            ...
            <form method="POST" action="{{ route('partner.products.update', $product) }}" enctype="multipart/form-data">
                @csrf @method('PUT')
                ...
                <div class="section">
                    ...
                    <select name="product_type" required>
                        @foreach($productTypes as $val => $type)
                            <option value="{{ $val }}" {{ old('product_type', $product->product_type) === $val ? 'selected' : '' }}>{{ $type['emoji'] }} {{ $type['label'] }}</option>
                        @endforeach
                    </select>
                    ...
                    <select name="style_type">
                        ...
                        @foreach($styleTypes as $val => $label)
                            <option value="{{ $val }}" {{ old('style_type', $product->style_type) === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    ...
                    <label>Kondisi</label>
                    <input type="text" name="condition" value="{{ old('condition', $product->condition) }}" required>
                </div>
                ...
                <div class="section">
                    ...
                    <textarea name="story" style="min-height:70px;">{{ old('story', $product->story) }}</textarea>
                </div>

                @php $existingChart = old('size_chart', $product->size_chart ?? []); @endphp
                <div class="section">
                    <p class="section-title">Panduan Ukuran (Size Chart)</p>
                    <div class="checkbox-row">
                        <input type="checkbox" id="has_size_chart" name="has_size_chart" value="1" {{ !empty($existingChart) ? 'checked' : '' }} onchange="toggleSizeChart(this)">
                        <label for="has_size_chart">Tambahkan panduan ukuran</label>
                    </div>
                    <div id="size-chart-wrap" style="{{ !empty($existingChart) ? '' : 'display:none;' }}margin-top:12px;">
                        <table class="size-chart-table">
                            ...
                            <tbody id="size-chart-body">
                                @forelse($existingChart ?: $sizeChartDefaults as $i => $row)
                                <tr>
                                    @foreach($sizeChartColumns as $key => $label)
                ...
                <div class="section">
                    ...
                    <p class="hint" style="margin-bottom:12px;">Foto saat ini:</p>
                    <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="preview-img" style="display:block;">

                    <label>Ganti dengan File Baru <span style="font-weight:400;color:#9ca3af">(opsional)</span></label>
                    <input type="file" name="image_file" accept="image/*">

                    <label>Atau Ganti dengan URL <span style="font-weight:400;color:#9ca3af">(opsional)</span></label>
                ...
                <div class="section">
                    ...
                    <label>Link Produk Tokopedia</label>
                    <input type="url" name="tokopedia_url" value="{{ old('tokopedia_url', $product->tokopedia_url) }}" placeholder="https://tokopedia.com/...">
                </div>

                <div class="section">
                    <p class="section-title">Varian Produk</p>
                    <div class="checkbox-row">
                        <input type="checkbox" id="has_variants" name="has_variants" value="1" onchange="toggleVariants(this)" {{ old('has_variants', $product->has_variants) ? 'checked' : '' }}>
                        <label for="has_variants">Produk ini memiliki varian (size/harga berbeda)</label>
                    </div>
                    <div id="variants-wrap" style="{{ old('has_variants', $product->has_variants) ? '' : 'display:none;' }}margin-top:12px;">
                        <table class="size-chart-table" style="margin-top:8px;">
                            <thead><tr><th>Ukuran</th><th>Harga (Rp)</th><th>Stok</th><th></th></tr></thead>
                            <tbody id="variants-body">
                                @forelse(old('variants', $product->variants ?? []) as $vi => $v)
                                <tr>
                                    <td><input type="text" name="variants[{{ $vi }}][size]" value="{{ $v['size'] ?? $v->size ?? '' }}"></td>
                                    <td><input type="number" name="variants[{{ $vi }}][price]" value="{{ $v['price'] ?? $v->price ?? '' }}" min="0"></td>
                                    <td><input type="number" name="variants[{{ $vi }}][stock]" value="{{ $v['stock'] ?? $v->stock ?? '' }}" min="0"></td>
                ...
                <div class="section">
                    ...
                    <textarea name="meta_description" style="min-height:50px;">{{ old('meta_description', $product->meta_description) }}</textarea>
                    <label>Meta Keywords <span style="font-weight:400;color:#9ca3af">(pisahkan dengan koma)</span></label>
                    <input type="text" name="meta_keywords" value="{{ old('meta_keywords', $product->meta_keywords) }}" placeholder="thrift, hoodie, preloved">
                </div>
                ...
                <div class="section">
                    ...
                    <div class="checkboxes">
                        ...
                        <div class="checkbox-row">
                            <input type="checkbox" id="is_sold" name="is_sold" value="1" {{ old('is_sold', $product->is_sold) ? 'checked' : '' }}>
    ...
    <script>
        ...
        let sizeRowIndex = {{ max(count($existingChart ?: $sizeChartDefaults), count($sizeChartDefaults)) }};
        function toggleSizeChart(cb) { document.getElementById('size-chart-wrap').style.display = cb.checked ? 'block' : 'none'; }
        function toggleVariants(cb) { document.getElementById('variants-wrap').style.display = cb.checked ? 'block' : 'none'; }
        let varRowIndex = {{ max(($product->variants->count() ?? 0), 1) }};
